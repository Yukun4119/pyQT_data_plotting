...
def get_img(s = 1, m = 1, year_index = 0, month_s_index = 0, month_e_index = 0, mode_index = 0):
	with open('..\\PyQT_data\\data\\%d.csv'%(int(s)),'r',encoding='UTF-8') as csvfile:
	...
	plt.title('%s-%s'%(eng_site_list[s-1], eng_material_list[m-1]), fontsize='large',fontweight='bold') 

	plt.ylabel('chemicals(ppb)')
	plt.xlabel('time')
